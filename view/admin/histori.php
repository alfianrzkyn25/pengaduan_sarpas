<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Histori Laporan</title><link rel="stylesheet" href="../assets/admin.css"></head><body>
...
<tr><th>ID</th><th>Aspirasi</th><th>Status Lama</th><th>Status Baru</th><th>Catatan</th><th>Oleh</th><th>Waktu</th></tr>
<?php while($h=mysqli_fetch_assoc($r)):?>
<tr><td><?=$h['id_histori']?></td><td><a class="btn secondary" style="padding:4px 8px" href="../controller/c_detail_aspirasi.php?id=<?=$h['id_aspirasi']?>">#<?=$h['id_aspirasi']?></a></td><td><?=e($h['status_lama']?:'-')?></td><td><span class="badge <?=strtolower($h['status_baru'])?>"><?=e($h['status_baru'])?></span></td><td><?=e($h['catatan']?:'-')?></td><td><?=e($h['diubah_oleh'])?></td><td><?=e(date('d-m-Y H:i',strtotime($h['waktu_ubah'])))?></td></tr>
<?php endwhile;?>
</table></div></section>
</main></div>
</body></html>
