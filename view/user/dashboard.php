...
<div class="heading"><div><p class="eyebrow">Portal Siswa</p><h1>Halo, <?=e($user['nama'])?> 👋</h1><p class="muted">Kelola pengaduan sarana dan lihat perkembangan laporanmu.</p></div><a class="btn" href="../controller/c_user_tambah.php">+ Buat Pengaduan</a></div>
<div class="grid">
...
<section class="panel"><div class="panel-head"><h2>Laporan Terbaru</h2><a href="../controller/c_user_laporan.php">Lihat semua</a></div>
<div class="list"><?php while($a=mysqli_fetch_assoc($r)): ?><a class="item" href="../controller/c_user_detail.php?id=<?=(int)$a['id_aspirasi']?>"><div><b>#<?=(int)$a['id_aspirasi']?> · <?=e($a['nama_kategori'])?></b><span><?=e($a['lokasi'])?></span><small><?=e($a['keterangan'])?></small></div><span class="badge <?=strtolower($a['status'])?>"><?=e($a['status'])?></span></a><?php endwhile; mysqli_stmt_close($st); ?></div>
</section>
</main></div>
</body></html>
